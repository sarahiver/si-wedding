...
const rotateCalendar = keyframes`
...
const Section = styled.section`
  ...
  ${p => p.$themeId === 'gold' && css`background: linear-gradient(180deg, #0D0B08 0%, #0A0A0A 100%);`}
  ${p => p.$themeId === 'botanical' && css`background: #2D3B2D;`}
  ${p => p.$themeId === 'contemporary' && css`background: #FFE66D;`}
  ${p => p.$themeId === 'luxe' && css`background: #FFFFFF;`}
  ${p => p.$themeId === 'neon' && css`background: linear-gradient(180deg, #0a0a0f 0%, #0f0a15 100%);`}
`;
...
const Subtitle = styled.p`
  ...
  ${p => p.$themeId === 'botanical' && css`font-family: 'Lato', sans-serif; font-size: 1.1rem; color: rgba(255,255,255,0.7);`}
  ${p => p.$themeId === 'contemporary' && css`font-family: 'Space Grotesk', sans-serif; font-size: 1.1rem; color: rgba(0,0,0,0.6);`}
  ${p => p.$themeId === 'luxe' && css`font-family: 'Montserrat', sans-serif; font-size: 0.95rem; color: #888;`}
...
const GoldPreview = styled.div`
  ...
  &::after { bottom: 20px; right: 20px; animation-delay: 1s; }
`;
...
const PricingBox = styled.div`
  ...
  ${p => p.$themeId === 'gold' && css`
    background: ${p.$included ? 'linear-gradient(135deg, rgba(212,175,55,0.2), rgba(212,175,55,0.05))' : 'rgba(255,255,255,0.02)'};
  ...
  ${p => p.$themeId === 'botanical' && css`
    ...
    .package {
      ...
      font-size: 0.75rem;
      font-weight: 600;
      color: ${p.$included ? 'rgba(255,255,255,0.8)' : 'rgba(255,255,255,0.5)'};
      margin-bottom: 10px;
    }
  ...
  ${p => p.$themeId === 'neon' && css`
    background: ${p.$included ? 'rgba(0,255,255,0.1)' : 'rgba(255,255,255,0.02)'};
    ...
    .price {
      ...
      ${p.$included && css`text-shadow: 0 0 15px rgba(0,255,255,0.5);`}
    }
...
const IncludedBadge = styled.div`
  ...
  ${p => p.$themeId === 'editorial' && css`background: #FF6B6B; color: #FFFFFF;`}
  ${p => p.$themeId === 'gold' && css`background: #D4AF37; color: #0A0A0A;`}
  ${p => p.$themeId === 'botanical' && css`background: #FFFFFF; color: #2D3B2D; border-radius: 10px;`}
  ${p => p.$themeId === 'contemporary' && css`background: #FF6B6B; color: #FFFFFF;`}
  ${p => p.$themeId === 'luxe' && css`background: #2A2A2A; color: #FFFFFF;`}
  ${p => p.$themeId === 'neon' && css`background: #ff00ff; color: #FFFFFF;`}
`;
...
function SaveTheDateSection() {
  ...
  const features = [
    { icon: '🚀', text: 'Sofort live – noch bevor eure Hauptwebsite fertig ist' },
  ...
  const pricingData = [
    ...
    { package: 'Luxe', price: 'Inklusive', note: 'Im Paket enthalten', included: true },
  ];
        ...
        return (
          <ContemporaryPreview>
            <ContemporaryPreviewContent>
              ...
              <div className="names">Sarah <span>&</span> Thomas</div>
              <div className="location">SCHLOSS HEIDELBERG // DEUTSCHLAND</div>
            </ContemporaryPreviewContent>
          </ContemporaryPreview>
        );
  ...
  const renderParallax = () => {
    switch (currentTheme) {
      case 'gold':
        return (
          <ParallaxBg>
            <GlowOrb $top="10%" $left="5%" $size="300px" $offset={parallaxOffset * 0.3} $color="rgba(212,175,55,0.08)" />
            <GlowOrb $top="60%" $right="10%" $size="250px" $offset={parallaxOffset * 0.5} $color="rgba(212,175,55,0.06)" />
          </ParallaxBg>
        );
      case 'botanical':
        return (
          <ParallaxBg>
            <FloatingElement $top="10%" $left="5%" $size="50px" $offset={parallaxOffset * 0.3} $opacity={0.2}>🌿</FloatingElement>
            <FloatingElement $top="40%" $right="8%" $size="40px" $offset={parallaxOffset * 0.5} $delay="1s" $opacity={0.15}>🌸</FloatingElement>
            <FloatingElement $bottom="15%" $left="10%" $size="45px" $offset={parallaxOffset * 0.4} $delay="2s" $opacity={0.18}>💐</FloatingElement>
          </ParallaxBg>
        );
      case 'neon':
        return (
          <ParallaxBg>
            <GlowOrb $top="20%" $left="5%" $size="250px" $offset={parallaxOffset * 0.3} $color="rgba(0,255,255,0.05)" />
            <GlowOrb $top="50%" $right="10%" $size="200px" $offset={parallaxOffset * 0.5} $color="rgba(255,0,255,0.05)" />
          </ParallaxBg>
        );
      default:
        return null;
      ...
      <Container>
        <Header>
          ...
          <Title $themeId={currentTheme}>
            {currentTheme === 'contemporary' ? 'SAVE THE DATE PAGE' : 
             currentTheme === 'neon' ? 'SAVE_THE_DATE.exe' :
             'Save the Date Seite'}
          </Title>
          <Subtitle $themeId={currentTheme}>
            Baut schon vor dem Go-Live eurer Hauptwebsite Vorfreude auf. Eure persönliche Save the Date Seite läuft unter eurer finalen Domain.
          </Subtitle>
        </Header>
          ...
          <FeaturesContainer $themeId={currentTheme}>
            ...
            <PricingGrid>
              {pricingData.map((item, index) => (
                <PricingBox key={index} $themeId={currentTheme} $included={item.included}>
                  {item.included && <IncludedBadge $themeId={currentTheme}>Inklusive</IncludedBadge>}
